...
void Snake::Tick(Textbox& l_textbox) {
	if (m_snakeBody.empty()) {
		return;
	}
...
void Snake::Move() {
	...
	int gridSize_X = World::GetWindowSize().x / World::GetBlockSize();
	int gridSize_Y = World::GetWindowSize().y / World::GetBlockSize();
	if (m_dir == Direction::Left) {
		--head.position.x;
		if (head.position.x == 0) {
			head.position.x = gridSize_X - 2;
		}
	}
	else if (m_dir == Direction::Right) {
		++m_snakeBody[0].position.x;
		if (head.position.x == gridSize_X - 1) {
			head.position.x = 1;
		}
	}
		...
		if (head.position.y == 0) {
			head.position.y = gridSize_Y - 2;
		}
	}
	else if (m_dir == Direction::Down) {
		++m_snakeBody[0].position.y;
		if (head.position.y == gridSize_Y - 1) {
			head.position.y = 1;
		}
	}
}

void Snake::CheckCollision(Textbox& l_textbox) {
	...
	for (auto itr = m_snakeBody.begin() + 1; itr != m_snakeBody.end(); ++itr) {
		if (itr->position == head.position) {
			...
			Cut(segments);
			if (m_lives - 1 != -1) {
				l_textbox.Add("Amount of lives left: " + std::to_string(m_lives - 1));
			}
			break;
		}
	}
