#include "World.h"
#include "Snake.h"

sf::Vector2u World::m_windowSize {0, 0};
int World::m_blockSize{0};

World::World(sf::Vector2u l_windSize, Snake& l_player) {
	m_blockSize = 16;
	m_windowSize = l_windSize;
	
	RespawnApple(l_player);
	m_appleShape.setFillColor(sf::Color::Red);
	m_appleShape.setRadius(m_blockSize / 2);

	for (int i = 0; i < 4; ++i) {
		m_bounds[i].setFillColor(sf::Color(150, 0, 0));
		if (!((i + 1) % 2)) {
			m_bounds[i].setSize(sf::Vector2f(m_windowSize.x, m_blockSize));
		}
		else {
			m_bounds[i].setSize(sf::Vector2f(m_blockSize, m_windowSize.y));
		}

		if (i < 2) {
			m_bounds[i].setPosition(0, 0);
		}
		else {
			m_bounds[i].setOrigin(m_bounds[i].getSize());
			m_bounds[i].setPosition(sf::Vector2f(m_windowSize));
		}
	}
}

World::~World() {}

void World::RespawnApple(Snake& l_player) {
	int maxX = (m_windowSize.x / m_blockSize) - 2;
	int maxY = (m_windowSize.y / m_blockSize) - 2;
    int randX;
    int randY;

    while (true) {
        randX = rand() % maxX + 1;
        randY = rand() % maxY + 1;
        if (!isPositionInSnake(randX, randY, l_player)) {
            break;
        }
    }
	m_applePosition = sf::Vector2i(randX, randY);
	m_appleShape.setPosition(m_applePosition.x * m_blockSize, m_applePosition.y * m_blockSize);
}

void World::Update(Snake& l_player, Textbox& l_textbox) {
	if (l_player.GetPosition() == m_applePosition) {
		l_player.Extend();
		l_player.IncreaseScore();
		l_textbox.Add("You ate an apple. Score: " + std::to_string(l_player.GetScore()));
		RespawnApple(l_player);
	}

	int gridSize_x = m_windowSize.x / m_blockSize;
	int gridSize_y = m_windowSize.y / m_blockSize;

	if (l_player.GetPosition().x <= 0 || l_player.GetPosition().y <= 0 ||
		l_player.GetPosition().x >= gridSize_x - 1 || l_player.GetPosition().y >= gridSize_y)
	{
		//l_player.Lose();
	}
}

void World::Render(sf::RenderWindow& l_window) {
	for (int i = 0; i < 4; ++i) {
		l_window.draw(m_bounds[i]);
	}
	l_window.draw(m_appleShape);
}

sf::Vector2u World::GetWindowSize()
{
	return m_windowSize;
}
int World::GetBlockSize() {
	return m_blockSize;
}

bool World::isPositionInSnake(int x, int y, Snake &l_player)
{
    auto position = sf::Vector2i(x, y);

    bool isBegin = true;
    for (const auto &value : l_player.m_snakeBody) {
        if (isBegin) {
            isBegin = false;
            continue;
        }
        if (value.position == position) {
            return true;
        }
    }

    return false;
}
